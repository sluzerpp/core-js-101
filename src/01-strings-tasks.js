...
function getRectangleString(width, height) {
  ...
  const top = `┌${multiply('─', width - 2)}┐\n`;
  let center = '';
  for (let i = 0; i < height - 2; i += 1) {
    center += `│${multiply(' ', width - 2)}│\n`;
  }
  const bottom = `└${multiply('─', width - 2)}┘\n`;
...
function encodeToRot13(str) {
  ...
  const alpCiph = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!.,';
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    newStr += alpCiph[alp.indexOf(str[i])];
  }
  return newStr;
}
